...
GitHub Pages で公開するブラウザツール集として運用中。明示された開発中機能や合意済みロードマップは、リポジトリ内では確認できない。

## Implemented

- ルート `index.html` には 78 ツールが登録され、対応する 78 ディレクトリすべてに `index.html` がある。
...
- 75 ツールに `test.js` があり、ルートの `site.test.js` とともに Node.js 組み込み機能だけで実行できる。
...
- README の表は 57 ツールで、ルート一覧の 78 ツールを網羅していない。
- `site.test.js` はカード数を 77 と期待するが、`index.html` には 78 件あるため失敗する。カード、ディレクトリ、各 `index.html` の対応は 78 件で一致する。
...
1. `site.test.js` の期待カード数について履歴またはオーナーの意図を確認し、全体テストを green に戻す。
2. README に未掲載の 21 ツールをルート一覧と照合し、利用者向け一覧を同期する。
3. YAML Viewer の CDN 依存を許容するかローカル化するか確認し、判断できた場合だけ ADR と説明を更新する。
4. 拡張子なしの空 `test` 2 件がテスト予定なのか不要ファイルなのか確認する。
5. 運用主体と必要性が明確になった場合、全 Node.js テストを実行する CI を検討する。
...
- README とルート一覧が 21 件ずれた経緯、および `site.test.js` の期待値が 77 のままである理由。
